...
_CLOSING_AUCTION_WINDOW_MINUTES = {"cn": 3, "hk": 10, "us": 5, "kr": 5}
...
def get_market_for_stock(code: str) -> Optional[str]:
    ...
    Returns:
        'cn' | 'hk' | 'us' | 'kr' | None (None = unrecognized, fail-open: treat as open)
    """
    ...
    from data_provider.base import _is_kr_market
...
def get_open_markets_today() -> Set[str]:
    ...
    Returns:
        Set of market keys ('cn', 'hk', 'us', 'kr') that are trading today
    """
    if not _XCALS_AVAILABLE:
        return {"cn", "hk", "us", "kr"}
    result: Set[str] = set()
